...
The airgap export bundle is pre-built in the background after the security DB changes (and at startup) under `BONGSU_TMPDIR`, keyed by the security DB revision, so downloads stream the cached file (time-to-first-byte drops from tens of seconds to near zero); a revision change from a sync or import invalidates the cache so a stale bundle is never served, and a cache miss builds on demand and repopulates it.
