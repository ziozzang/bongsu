...
Set `BONGSU_RELEASE_READINESS_REPORT` during release or handoff runs to persist a JSON evidence report with the source commit, selected options, each gate command, exit status, timestamps, duration, total failed gate count, and the `BONGSU_RELEASE_ARCHIVE` SHA-256 plus sidecar checksum match state when an archive is verified. Keep this report with the release archive, CI artifact, or handoff notes so a failed promotion has an auditable record instead of only terminal output.
...
In connected live environments it also compares selected OSV upstream `all.zip` `Last-Modified` headers with local OSV freshness. With `BONGSU_DB_DSN`, each sentinel ecosystem's raw OSV row timestamp is authoritative, with an indexed matchable-row fallback only for affected-product-only ecosystems such as Wolfi; the affected-package index timestamp is shown separately as index rebuild freshness. This lets partial ecosystem recovery be validated without promoting aggregate source freshness or confusing a global index rebuild with a source refresh. Without DB access, the gate falls back to the aggregate local OSV source timestamp. The default and release-readiness grace is `BONGSU_VERIFY_CVEDB_OSV_UPSTREAM_GRACE_SECONDS=3600`, short enough to catch source-specific OSV lag inside the 6-hour sync cadence while tolerating feed updates that land during a full OSV import.
...
It verifies airgap bundle readiness with `./scripts/verify-live-security-db-export-freshness.sh`: `/api/admin/security-db/status` must report `security_db_export.status == "ok"`, no `outdated_sources`, complete `latest_exported_at` / `latest_source_update_at` timestamps (computed from source registry and raw CVE row updates), and healthy `security_db_freshness` by default. If this gate fails, export a new security DB bundle before moving data into an air-gapped deployment.

Expect the bundle export to spend ~30–90 seconds (more with the Trivy DB included) before the download starts: the server materializes and checksums the complete archive to a temporary file first so a failed build can never produce a partial download. The dashboard shows a preparing message during this window; from curl, time-to-first-byte equals the preparation time and the transfer then completes at line speed.
...
It verifies fixture cleanup hygiene with `./scripts/verify-live-fixture-cleanup.sh`: live verifier scripts that create `host-*` fixtures must install `trap cleanup EXIT` and visibly remove fixture hosts through the host delete API or explicit database cleanup, live verifier scripts that create RBAC subjects or policies must visibly delete those RBAC fixtures, and live verifier scripts that create scan-request fixtures must visibly cancel or remove those queue entries. This keeps repeated release gates from leaving synthetic hosts that degrade agent-fleet health, stale access policies that pollute authorization tests, or abandoned pending/claimed scan requests that inflate force-scan and automatic rescan backlogs.
...
CVE search is bounded for responsiveness on a production-sized database: the result count is capped at `BONGSU_CVE_SEARCH_COUNT_CAP` (default `10000`, shown verbatim in the UI when a query matches more), the count and data queries run concurrently, and each runs with `BONGSU_CVE_SEARCH_WORK_MEM` (default `128MB`) so the multi-column union de-duplication stays in memory instead of spilling to a temp file. Per-key reference-group summaries are served from the materialized `cve_reference_group_summary` table, refreshed at the end of every reference-index rebuild, so search pages do not join the full reference-key index live.

The airgap export bundle is built on demand and cached: the first download builds it and caches the result keyed by the security DB revision, and subsequent downloads at the same revision stream the cached file; a revision change from a sync or import invalidates the cache. Optionally set `BONGSU_SECDB_BUNDLE_PREBUILD=true` to pre-build the bundle in the background after each security DB change so even the first download is instant. The bundle's intermediate JSONL is larger than 1 GB, so the export, the bundle tarball, and multipart upload spillover are all routed to `BONGSU_TMPDIR` (TMPDIR is set process-wide); point `BONGSU_TMPDIR` at storage with several GB free so exports do not fail on a size-limited `/tmp` tmpfs.

Bundle import accepts either a `multipart/form-data` `bundle` field or the raw `.tar.gz` as the request body (`Content-Type: application/gzip`); the dashboard and `curl --data-binary @bundle.tar.gz` both use the raw-body path so a ~170 MB+ bundle streams straight into the importer without buffering. The CVE-row replacement runs under a detached, timeout-bounded context (`BONGSU_SECURITY_DB_IMPORT_TIMEOUT_SECONDS`, default `1800`) with extended request read/write deadlines, so a slow airgap upload or the multi-minute database replace is not cut off and a brief client disconnect cannot roll it back.
...
- Verify the real enrolled agent fleet before staging or production promotion:
...
This is not a fixture verifier. It checks `/api/admin/agent-fleet/status`, `/api/stats`, and `/api/hosts` for real enrolled hosts after excluding verifier-style fixture IDs. By default it requires installer readiness, at least one online real host, no stale/offline/outdated/unknown agents, no fleet warnings, 100% fresh inventory coverage, completed latest inventories with package evidence, no pending scan requests, and 100% host coverage on the current `security_db_revision`. Tune thresholds with `BONGSU_VERIFY_AGENT_FLEET_MIN_HOSTS`, `BONGSU_VERIFY_AGENT_FLEET_MIN_ONLINE_HOSTS`, `BONGSU_VERIFY_AGENT_FLEET_MAX_DEGRADED_INVENTORIES`, `BONGSU_VERIFY_AGENT_FLEET_ALLOWED_SCAN_STATUSES`, `BONGSU_VERIFY_AGENT_FLEET_MAX_PENDING_SCAN_REQUESTS`, `BONGSU_VERIFY_AGENT_FLEET_MAX_WARNINGS`, and `BONGSU_VERIFY_AGENT_FLEET_MIN_SECURITY_DB_SCAN_COVERAGE_PERCENT` only when documenting a deliberate staged rollout exception.
...
For connected security DB syncs, keep OSV ecosystem chunks in append/upsert mode and defer finalization until the OSV loop finishes. `scripts/sync-all-cvedb.sh` sends `replace=false` and `finalize=false` for each OSV chunk because all chunks share `source=osv`; importing each chunk with source replacement enabled leaves only the last ecosystem in the live CVE DB, and finalizing every chunk repeats expensive affected/reference index rebuilds and recalculation. Deferred OSV chunk imports do not refresh the aggregate `security_sources.osv` registry row; only a finalized import or successful source-wide stale-prune after the complete OSV loop may publish OSV source freshness. OSV rows can also share the same CVE alias across different ecosystems, so the CVE upsert path merges existing and incoming `affected_products`/`refs` arrays instead of overwriting the previous row. This keeps examples such as `phenx/php-svg-lib` / `Packagist` / fixed `0.5.2` matchable even when a later distro chunk references the same CVE. After all OSV chunks succeed, the script prunes `source=osv` rows older than the sync start timestamp so upstream deletions are reflected, then queues asynchronous affected-package and reference-key index rebuilds, polls `/api/health` for completion, queues security recalculation once, and runs the strict OSV upstream freshness verifier by default. Partial OSV syncs skip source-wide stale prune and only run that post-sync verifier when `BONGSU_DB_DSN` is available, because DB-backed raw ecosystem `updated_at` values can prove selected ecosystem freshness without promoting aggregate OSV source freshness. Tune the index wait loop with `BONGSU_CVE_INDEX_REBUILD_WAIT_SECONDS` (default `900`) and `BONGSU_CVE_INDEX_REBUILD_POLL_SECONDS` (default `5`) for very large imports; tune the preflight threshold with `BONGSU_CVE_INDEX_REBUILD_MIN_SERVER_TIMEOUT_SECONDS` (default `600`) when a deployment intentionally uses smaller feeds. Set `BONGSU_OSV_POST_SYNC_VERIFY=false` only for emergency diagnostics where the sync artifact must be inspected separately. Non-OSV sources still use source replacement and immediate finalization by default.

### Incremental security DB sync

`scripts/sync-all-cvedb.sh` runs **incrementally by default** and performs a **full** sync at most every `BONGSU_SYNC_FULL_INTERVAL_DAYS` (default `7`), tracked by the marker file `BONGSU_SYNC_FULL_MARKER` (default `<BONGSU_TMPDIR>/.bongsu-last-full-sync`). Force a full sync with `BONGSU_SYNC_FULL=1`. Each source is fetched only from its server-reported watermark (`GET /api/admin/cve-db/source/{source}/watermark` → `MAX(modified_date)`) minus an overlap window `BONGSU_SYNC_OVERLAP_HOURS` (default `48`, absorbs clock skew and back-dated upstream edits):

- **NVD** — incremental uses the NVD `lastModStartDate` window (chunked to the API's 120-day max) and UPSERTs the delta (`replace=false`); a full sync re-pulls the `pubDate` year window and replaces the source (`replace=true`). A typical daily delta is a few hundred records versus ~150k for the full window.
- **OSV** — OSV.dev only publishes full per-ecosystem `all.zip` dumps, so the download is always full, but incremental writes only entries with `modified >= watermark` so the import (the contended, index/recalc-heavy step) processes the delta only. Stale-prune is **full-only** because it deletes any OSV row not refreshed in the run, which is correct only when every ecosystem was fully re-imported.
- **EPSS** — a full daily snapshot with no delta feed; skipped entirely when the watermark date is already today, and the import is skipped when the fetched file's date is not newer than the watermark (e.g. today's file is not yet published).
- **CISA KEV** and **Trivy** — always imported in full. They are small, and source freshness is derived from the newest row's `updated_at`, so a full re-import keeps their freshness green; skipping them would save nothing meaningful and let them age out.

Incremental runs never advance the full-sync marker, so the periodic full sync (with its upstream-deletion prune) is always guaranteed within the interval. Because incremental adds/updates only, **deletions are reconciled only by the periodic full sync** — keep `BONGSU_SYNC_FULL_INTERVAL_DAYS` at or below your tolerance for stale-but-removed advisories. Source freshness for the `30h` (`BONGSU_SECURITY_DB_MAX_SOURCE_AGE_HOURS`) staleness gate continues to reflect real data recency: NVD/OSV deltas land daily and KEV/Trivy/EPSS are re-imported, so all sources stay within the window in normal operation; a source flagged stale means its upstream genuinely produced nothing for that window, which is intentionally surfaced rather than masked.

For connected security DB syncs, keep OSV ecosystem chunks in append/upsert mode and defer finalization until the OSV loop finishes. `scripts/sync-all-cvedb.sh` sends `replace=false` and `finalize=false` for each OSV chunk because all chunks share `source=osv`; importing each chunk with source replacement enabled leaves only the last ecosystem in the live CVE DB, and finalizing every chunk repeats expensive affected/reference index rebuilds and recalculation. Deferred OSV chunk imports do not refresh the aggregate `security_sources.osv` registry row; only a finalized import or successful source-wide stale-prune after the complete OSV loop may publish OSV source freshness. OSV rows can also share the same CVE alias across different ecosystems, so the CVE upsert path merges existing and incoming `affected_products`/`refs` arrays instead of overwriting the previous row. This keeps examples such as `phenx/php-svg-lib` / `Packagist` / fixed `0.5.2` matchable even when a later distro chunk references the same CVE. After all OSV chunks succeed, a full sync prunes `source=osv` rows older than the sync start timestamp so upstream deletions are reflected, then queues asynchronous affected-package and reference-key index rebuilds, polls `/api/health?fresh=true` for completion (the `fresh=true` bypass avoids the short-TTL health cache so a just-queued rebuild is never misread as not-running), queues security recalculation once, and runs the strict OSV upstream freshness verifier by default. Partial OSV syncs skip source-wide stale prune and only run that post-sync verifier when `BONGSU_DB_DSN` is available, because DB-backed raw ecosystem `updated_at` values can prove selected ecosystem freshness without promoting aggregate OSV source freshness. Tune the index wait loop with `BONGSU_CVE_INDEX_REBUILD_WAIT_SECONDS` (default `900`) and `BONGSU_CVE_INDEX_REBUILD_POLL_SECONDS` (default `5`) for very large imports; tune the preflight threshold with `BONGSU_CVE_INDEX_REBUILD_MIN_SERVER_TIMEOUT_SECONDS` (default `600`) when a deployment intentionally uses smaller feeds. Set `BONGSU_OSV_POST_SYNC_VERIFY=false` only for emergency diagnostics where the sync artifact must be inspected separately. Per-source imports always defer finalization (`finalize=false`) and a single finalization pass runs once at the end of the sync, so no background recalculation overlaps a later import transaction (the source of the historic `cve_database` deadlock, SQLSTATE 40P01). Imports additionally retry the whole transaction on transient deadlock/serialization failures.
...
5. Check `/api/health`, `/api/cve-db/stats?refresh=true`, and the dashboard CVE DB status card. `/api/health` uses indexed-only CVE index summaries for liveness; use stats, admin security DB status, and `./scripts/verify-live-cvedb-quality.sh` for detailed affected/reference coverage and OSV upstream freshness. In the stats response, review `osv_ecosystems` for the largest OSV ecosystems: `raw_last_update` tracks the source row freshness used for upstream checks, while `indexed_last_update` tracks the affected-package index rebuild time.
...
For large backups, restores, Trivy DB downloads, or connected CVE source syncs on hosts with a small `/tmp`, set `BONGSU_TMPDIR=/path/with/space` before running the packaged scripts. The backup, restore, Trivy DB download, NVD/OSV/Trivy CVE sync, OSV download, and Trivy CVE extraction scripts create managed `bongsu-*` work directories under that path and remove them on exit. OSV downloads also check temporary free space before download and extraction; the default minimum is 8 GiB and can be tuned with `BONGSU_OSV_MIN_FREE_BYTES` after confirming the selected `BONGSU_OSV_ECOSYSTEMS` fit.
...
The OSV-only refresh uses the same per-ecosystem append/upsert mode as the full connected sync, prunes stale `source=osv` rows only after every configured ecosystem succeeds, then queues asynchronous affected-package and reference-key rebuilds, waits for them to finish through `/api/health`, queues security recalculation once, and runs the strict OSV upstream freshness verifier by default. For a single lagging upstream sentinel, run the same script with both `BONGSU_OSV_ECOSYSTEMS=<ecosystem>` and `BONGSU_DB_DSN=...`; partial overrides intentionally skip source-wide stale prune and do not promote aggregate OSV source freshness, so DB-backed ecosystem freshness is required for the post-sync verifier. Without `BONGSU_DB_DSN`, the script will complete the partial refresh but print an explicit post-sync freshness verification skip.
