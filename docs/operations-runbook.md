...
In connected live environments it also compares selected OSV upstream `all.zip` `Last-Modified` headers with local OSV freshness. With `BONGSU_DB_DSN`, each sentinel ecosystem's affected-package index timestamp is authoritative, so partial ecosystem recovery can be validated without promoting aggregate source freshness. Without DB access, the gate falls back to the aggregate local OSV source timestamp. The default and release-readiness grace is `BONGSU_VERIFY_CVEDB_OSV_UPSTREAM_GRACE_SECONDS=3600`, short enough to catch source-specific OSV lag inside the 6-hour sync cadence while tolerating feed updates that land during a full OSV import.
