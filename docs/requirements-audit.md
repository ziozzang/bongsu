...
| R4 | RBAC by UserID/GroupID for host/container/system access. | Implemented | access subject/policy migrations, `/api/admin/rbac-*` handlers, dashboard RBAC view, viewer-key flow | `go test ./...`; multi-tenant policy examples need broader scenario tests |
...
| R6 | Deploy fully in air-gapped environments; update externally, export, import internally. | Verified | airgap compose file, bundle import/export, package script, static binary verification, trivy archive validation | `./scripts/verify-deploy-config.sh`, `./scripts/verify-static-binaries.sh`, compose config checks |
| R7 | Provide force scan and rematch functions. | Implemented | scan request APIs, agent daemon polling, CVE rematch endpoints, auto-rescan on security DB changes | `go test ./...`; live force-scan operator flow still needs browser/API smoke coverage |
...
| R11 | Everything must be sufficiently tested. | Partial | Go unit/source tests, Playwright E2E, deployment/static/installer/migration verifiers, GitHub Actions CI | CI and local verifier suite; broader live multi-host/container fixtures remain useful |
...
- Multi-host/container force-scan and RBAC scenarios need stronger end-to-end fixtures.
...
- Operational docs should be expanded into install, upgrade, backup/restore, alerting, and incident-response runbooks before a real commercial release.
