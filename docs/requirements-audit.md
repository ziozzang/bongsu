...
- Live browser smoke is automated through `./scripts/verify-live-web-smoke.sh` and currently covers dashboard CVE DB status plus CVE Search, Hosts, Packages, Containers, Scan History, Vulnerabilities, RBAC, Audit Log, Schedules, Asset Groups, Trends, Reports, and Notifications on the deployed `5678` web UI while failing on any `/api/` 5xx response; repeat it after larger UI changes and still do a staging visual pass before commercial release.
