...
| R7 | Provide force scan and rematch functions. | Implemented | scan request APIs, dashboard force-scan buttons, asset-group scan trigger, scheduled scans, agent daemon polling/retry, CVE rematch endpoints, auto-rescan on security DB changes | `go test ./...`, `npm --prefix web run test:e2e`; live force-scan against enrolled agents remains environment-specific |
...
| R19 | Maintain documented, operator-usable API and recovery surfaces as the product grows. | Implemented | embedded and docs OpenAPI 3.1 specs, OpenAPI verifier, backup/restore scripts, CHANGELOG, report/trend/notification/admin endpoints | `./scripts/verify-openapi.sh`, `go test ./...` |
...
- Schedules, asset groups, reports, and notification rules now have mocked browser workflow coverage with request payload assertions; live operator workflow coverage should still exercise them against a real server and enrolled agents before a commercial release.
- Local sessions, backup/restore, and OpenAPI documentation surfaces have unit/source coverage and docs, but should still be exercised in a realistic operator workflow before a commercial release.
