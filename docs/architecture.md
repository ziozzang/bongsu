...
Current implementation exposes request creation/listing/cancel plus agent claim/complete endpoints. Agents can run with `--daemon --poll-interval 60s` to claim pending requests, execute a scan, upload the report, and mark the request completed or failed. Claimed requests older than `BONGSU_SCAN_REQUEST_CLAIM_TIMEOUT_MINUTES` are requeued automatically during agent claim and can also be requeued by admins from the Scan History view. Security DB updates enqueue `security-db-update` scan requests automatically, deduplicated against existing pending or claimed work per host. Viewer access to scan request queues is host-scoped through RBAC; global all-host requests remain visible only to admins.
