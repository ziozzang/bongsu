...
func TestSecuritySourceRegistryStatusTracksImportedCveSources(t *testing.T) {
	...
	for _, want := range []string{
		...
		"func (db *DB) EnsureSecuritySourceStatusTx",
		"func (db *DB) refreshSecuritySourceStatusTx",
		"func (db *DB) RefreshSecuritySourceStatus",
		"func (db *DB) EnsureSecuritySourceStatus",
		"func (db *DB) MarkSecuritySourcesExportedTx",
		"func (db *DB) MarkSecuritySourcesExported",
		...
		"preserveExistingSync",
		"COALESCE(security_sources.last_sync_finished_at, EXCLUDED.last_sync_finished_at)",
...
func TestListVulnerabilitiesExposesCisaKevPrioritization(t *testing.T) {
	...
	for _, want := range []string{
		...
		"&v.RiskLevel",
		`FROM cve_kev kev`,
		`e.score FROM cve_epss`,
		`e.percentile FROM cve_epss`,
