...
func TestAdminMetricsExposeAgentFleetState(t *testing.T) {
	...
	for _, want := range []string{
		...
		"CountStaleScanRequestsByState(ctx, nil, true, scanRequestClaimTimeoutSeconds())",
		"CountSecurityDBRescanRequestsByStatus(ctx, nil, true, revision)",
		"CountStaleSecurityDBRescanRequestsByState(ctx, nil, true, revision, scanRequestClaimTimeoutSeconds())",
		"GetSecurityDBScanCoverage(ctx, nil, true, revision)",
		"agentFleetOperationalStatus(len(hosts), agentStatusCounts, driftCounts, inventoryStatusCounts, staleScanRequestCounts, securityDBRescanCounts, securityDBRescanStaleCounts, securityDBScanCoverage, s.installToken != \"\", agentInstaller, trivyInstaller)",
...
func TestLiveAgentFleetRolloutVerifierChecksRealFleetReadiness(t *testing.T) {
	...
	for _, want := range []string{
		...
		`BONGSU_VERIFY_AGENT_FLEET_MAX_WARNINGS`,
		`BONGSU_VERIFY_AGENT_FLEET_EXCLUDE_HOST_REGEX`,
		`BONGSU_VERIFY_AGENT_FLEET_ALLOWED_SCAN_STATUSES`,
		`agent fleet status must be ok unless warnings are explicitly allowed`,
...
func TestAgentFleetOperationalStatusDegradesOnActionableIssues(t *testing.T) {
	status, warnings, actions := agentFleetOperationalStatus(
		...
		map[string]int{"healthy": 0, "degraded": 1, "stale": 1, "empty": 1, "none": 1},
		map[string]int{"pending": 1, "claimed": 1},
		map[string]int{"pending": 1, "claimed": 1},
		map[string]int{"pending": 1, "claimed": 1},
		&db.SecurityDBScanCoverage{TotalHosts: 3, CurrentHosts: 1, StaleHosts: 2, CoveragePercent: 33.3},
		false,
	...
	for _, want := range []string{
		...
		"one or more hosts have degraded latest inventory",
		"one or more hosts have stale latest inventory",
		"one or more hosts have no usable package inventory",
		"one or more scan requests are stale",
		"current security DB rescans are stale",
		"current security DB rescans are still open",
		"not all hosts have reported against the current security DB revision",
	} {
	...
	status, warnings, actions = agentFleetOperationalStatus(
		...
		map[string]int{"healthy": 1, "degraded": 0, "stale": 0, "empty": 0, "none": 0},
		map[string]int{"pending": 0, "claimed": 0},
		map[string]int{"pending": 0, "claimed": 0},
		map[string]int{"pending": 0, "claimed": 0},
		&db.SecurityDBScanCoverage{TotalHosts: 1, CurrentHosts: 1, CoveragePercent: 100},
		true,
...
func TestVulnerabilityExportRequiresExportScopeAndAuditsBeforeWrite(t *testing.T) {
	...
	for _, want := range []string{
		...
		`envInt("BONGSU_VULNERABILITY_EXPORT_TIMEOUT_SECONDS", 60)`,
		"context.WithTimeout(r.Context(), time.Duration(timeout)*time.Second)",
		"s.db.ListVulnerabilities(exportCtx, filter, maxRows, 0)",
		"errors.Is(err, context.DeadlineExceeded)",
		"errors.Is(exportCtx.Err(), context.DeadlineExceeded)",
		`http.StatusGatewayTimeout`,
		`"vulnerability export timeout"`,
		`"db lookup timed out"`,
		`"timeout_seconds"`,
		"revisionMeta := s.securityDBRevisionMeta(r.Context())",
...
func TestSecurityDBFreshnessHealthAndMetricsAreExposed(t *testing.T) {
	...
	for _, want := range []string{
		...
		`bongsu_security_source_registry_records_total`,
...
func TestAdminMetricsExposeCveSourceQuality(t *testing.T) {
	...
	for _, want := range []string{
		...
		"ListSecuritySourceStatuses(ctx)",
		"bongsu_security_source_registry_enabled",
		"bongsu_security_source_registry_ok",
		...
		"bongsu_security_source_registry_ok_sources",
		"bongsu_security_source_registry_records_total",
		"bongsu_security_source_registry_metrics_error",
		...
		"GetCvePlaceholderStats(ctx)",
		"buildCveDBQualitySummary(cveDBQualityInput{",
...
func TestCveDbImportRefreshesSecuritySourceRegistry(t *testing.T) {
	...
	for _, want := range []string{
		...
		// The bundle import refreshes the registry inside applySecurityDBBundle,
		// which runs under a detached import context (decoupled from the client
		// connection) shared by the sync and async paths.
		`RefreshSecuritySourceStatusTx(ctx, tx, "")`,
...
func TestOsvDownloaderFailsClosedAndWritesAtomically(t *testing.T) {
	...
	for _, want := range []string{
		...
		`BONGSU_OSV_MIN_FREE_BYTES`,
		`required_bytes=${OSV_MIN_FREE_BYTES}`,
		`Set BONGSU_TMPDIR to a filesystem with more free space`,
...
func TestLiveCveDbQualityVerifierChecksMatchableSentinelAndFixedVersionQuality(t *testing.T) {
	...
	for _, want := range []string{
		...
		`fixed_version ~ '/'`,
		"affected package index rows must keep version-like fixed versions only",
...
func TestReleaseReadinessReportVerifierExercisesSuccessAndFailureReports(t *testing.T) {
	...
	for _, want := range []string{
		...
		`.ci.github_actions == true and .ci.workflow == "CI"`,
		`.status == "passed" and .exit_code == 0`,
		`.status == "failed" and .exit_code == 7`,
		`.failed_gate_count == 1`,
		`.options.live == true and .options.require_db == true and .options.skip_heavy == true`,
		`any(.gates[]; .kind == "exec" and .name == "go test ./...")`,
		`any(.gates[]; .kind == "exec" and .name == "./scripts/verify-operator-workflow.sh")`,
		`any(.gates[]; .kind == "exec" and .name == "./scripts/verify-live-retention-prune.sh")`,
		`BONGSU_VERIFY_CVEDB_REQUIRE_FRESH_SOURCES=true`,
		`BONGSU_VERIFY_CVEDB_REQUIRE_DB=true`,
...
func TestLiveServerBuildVerifierChecksSourceAlignedServerBuild(t *testing.T) {
	...
	for _, want := range []string{
		...
		`git log -1 --format=%H --`,
		`"${SERVER_BUILD_FILES[@]}"`,
		`merge-base --is-ancestor "$EXPECTED_SERVER_COMMIT" "$server_commit"`,
		`diff --quiet "$EXPECTED_SERVER_COMMIT" "$server_commit" -- "${SERVER_BUILD_FILES[@]}"`,
		`server build inputs are unchanged`,
		`cmd/server`,
		`internal/server`,
...
func TestLiveCveRematchWorkflowVerifierUsesPackagistFixture(t *testing.T) {
	...
	for _, want := range []string{
		...
		`BONGSU_DB_DSN is required for live CVE rematch DB evidence verification`,
		`FROM cve_affected_packages cap`,
		`cap.vulnerability_id = r.vulnerability_id`,
		`cap.package_name = r.package_name`,
		`cap.ecosystem = r.package_ecosystem`,
		`cap.fixed_version = r.fixed_version`,
		`every generated cve-db finding must be backed by matching affected-package evidence`,
...
func TestSecurityDBRevisionMetaUsesBoundedCacheAndInvalidation(t *testing.T) {
	...
	for _, want := range []string{
		...
		"context.WithTimeout(context.Background(), securityDBRevisionQueryTimeout())",
		`meta["security_db_revision"]`,
		"generation == s.securityDBRevisionCacheGen",
