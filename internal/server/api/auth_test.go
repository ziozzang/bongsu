...
func TestAdminMetricsExposeAgentFleetState(t *testing.T) {
	...
	for _, want := range []string{
		...
		"agentFleetOperationalStatus(len(hosts), agentStatusCounts, driftCounts, staleScanRequestCounts, securityDBRescanCounts, securityDBRescanStaleCounts, securityDBScanCoverage, s.installToken != \"\", agentInstaller, trivyInstaller)",
