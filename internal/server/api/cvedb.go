...
func (s *Server) runAffectedIndexRebuild() {
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(envInt("BONGSU_CVE_AFFECTED_INDEX_REBUILD_TIMEOUT_SECONDS", 900))*time.Second)
	defer cancel()
...
func (s *Server) runReferenceIndexRebuild() {
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(envInt("BONGSU_CVE_REFERENCE_INDEX_REBUILD_TIMEOUT_SECONDS", 900))*time.Second)
	defer cancel()
