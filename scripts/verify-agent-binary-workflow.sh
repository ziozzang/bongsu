...
PRIMARY_CONTAINER_NAME="bongsu-fixture-container-${HOST_ID_PRIMARY}"
SECONDARY_CONTAINER_NAME="bongsu-fixture-container-${HOST_ID_SECONDARY}"
PRIMARY_IMAGE_ID="sha256:fixture-image-${HOST_ID_PRIMARY}"
SECONDARY_IMAGE_ID="sha256:fixture-image-${HOST_ID_SECONDARY}"
...
assert_json_arg2 "$packages_json" container_id "$PRIMARY_CONTAINER_ID" image_name "fixture.registry/bongsu-agent-fixture:${HOST_ID_PRIMARY}" '.items[] | select(.name == "bongsu-container-fixture-package" and .asset_type == "container" and .asset_id == $container_id and .container_id == $container_id and .image_name == $image_name)' "container Trivy package must preserve container/image context"
assert_json_arg2 "$packages_json" container_name "$PRIMARY_CONTAINER_NAME" image_id "$PRIMARY_IMAGE_ID" '.items[] | select(.name == "bongsu-container-fixture-package" and .container == $container_name and .image_id == $image_id)' "container Trivy package must preserve container instance name and image ID"
assert_json_arg2 "$packages_json" container_id "$PRIMARY_CONTAINER_ID" image_name "fixture.registry/bongsu-agent-fixture:${HOST_ID_PRIMARY}" '.items[] | select(.name == "bongsu-container-python-library" and .asset_type == "container" and .asset_id == $container_id and .container_id == $container_id and .image_name == $image_name and .pkg_type == "python-pkg" and .ecosystem == "PyPI" and .purl == "pkg:pypi/bongsu-container-python-library@1.2.3" and .target == "app/requirements.txt")' "container Trivy code library must preserve PyPI ecosystem, purl, and container/image context"
assert_json_arg2 "$packages_json" container_name "$PRIMARY_CONTAINER_NAME" image_id "$PRIMARY_IMAGE_ID" '.items[] | select(.name == "bongsu-container-python-library" and .container == $container_name and .image_id == $image_id)' "container Trivy code library must preserve container instance name and image ID"
...
assert_json_arg2 "$containers_json" container_name "$PRIMARY_CONTAINER_NAME" image_id "$PRIMARY_IMAGE_ID" '.items[] | select(.name == $container_name and .image_id == $image_id and .runtime == "docker")' "container asset must preserve runtime name and image ID"
...
assert_json_arg2 "$secondary_packages_json" container_id "$SECONDARY_CONTAINER_ID" image_name "fixture.registry/bongsu-agent-fixture:${HOST_ID_SECONDARY}" '.items[] | select(.name == "bongsu-container-fixture-package" and .asset_type == "container" and .asset_id == $container_id and .container_id == $container_id and .image_name == $image_name)' "secondary container package must preserve secondary container/image context"
assert_json_arg2 "$secondary_packages_json" container_name "$SECONDARY_CONTAINER_NAME" image_id "$SECONDARY_IMAGE_ID" '.items[] | select(.name == "bongsu-container-fixture-package" and .container == $container_name and .image_id == $image_id)' "secondary container package must preserve secondary container instance name and image ID"
assert_json_arg2 "$secondary_packages_json" container_id "$SECONDARY_CONTAINER_ID" image_name "fixture.registry/bongsu-agent-fixture:${HOST_ID_SECONDARY}" '.items[] | select(.name == "bongsu-container-python-library" and .asset_type == "container" and .asset_id == $container_id and .container_id == $container_id and .image_name == $image_name and .ecosystem == "PyPI")' "secondary container code-library package must preserve secondary container/image context"
assert_json_arg2 "$secondary_packages_json" container_name "$SECONDARY_CONTAINER_NAME" image_id "$SECONDARY_IMAGE_ID" '.items[] | select(.name == "bongsu-container-python-library" and .container == $container_name and .image_id == $image_id)' "secondary container code-library package must preserve secondary container instance name and image ID"
...
secondary_containers_json="$(api_json GET "/api/containers?host_id=${HOST_ID_SECONDARY}&limit=20")"
assert_json_arg2 "$secondary_containers_json" container_id "$SECONDARY_CONTAINER_ID" image_name "fixture.registry/bongsu-agent-fixture:${HOST_ID_SECONDARY}" '.items[] | select(.container_id == $container_id and .image_name == $image_name and .state == "running")' "secondary container asset must be persisted"
assert_json_arg2 "$secondary_containers_json" container_name "$SECONDARY_CONTAINER_NAME" image_id "$SECONDARY_IMAGE_ID" '.items[] | select(.name == $container_name and .image_id == $image_id and .runtime == "docker")' "secondary container asset must preserve runtime name and image ID"
assert_json_arg "$secondary_containers_json" container_id "$PRIMARY_CONTAINER_ID" 'all(.items[]; .container_id != $container_id)' "secondary container query must not leak primary container assets"
