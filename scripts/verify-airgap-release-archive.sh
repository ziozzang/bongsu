...
for path in \
    ...
    "$ROOT_DIR/scripts/verify-live-report-export-rbac.sh" \
    "$ROOT_DIR/scripts/verify-live-sbom-export-rbac.sh" \
    "$ROOT_DIR/scripts/verify-live-sbom-export-workflow.sh" \
    "$ROOT_DIR/scripts/verify-live-scan-request-recovery.sh" \
    "$ROOT_DIR/scripts/verify-live-security-db-auto-rescan.sh" \
...
require_text "$ROOT_DIR/scripts/verify-security-db-bundle-file.sh" 'BONGSU_VERIFY_SECURITY_DB_BUNDLE_VALIDATE_JSONL' "packaged security DB bundle file verifier must expose JSONL parse validation control"
...
require_text "$ROOT_DIR/scripts/verify-security-db-bundle-file.sh" 'jq -c \.' "packaged security DB bundle file verifier must parse CVE JSONL records"
