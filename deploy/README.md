...
| `GET` | `/api/admin/metrics` | Admin-only Prometheus text metrics for DB pool health, agent status/version coverage, security recalculation state, Trivy readiness, security source freshness/quality, active risk-level backlog, and automatic rescan backlog |
